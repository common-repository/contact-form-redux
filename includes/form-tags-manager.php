...
function cfredux_add_form_tag($tag, $func, $features = '')
{
...
function cfredux_remove_form_tag($tag)
{
    $manager = CFREDUX_FormTagsManager::get_instance();
...
function cfredux_replace_all_form_tags($content)
{
    $manager = CFREDUX_FormTagsManager::get_instance();
...
function cfredux_scan_form_tags($cond = null)
{
    $contact_form = CFREDUX_ContactForm::get_current();
...
function cfredux_form_tag_supports($tag, $feature)
{
    $manager = CFREDUX_FormTagsManager::get_instance();
...
class CFREDUX_FormTagsManager
{
    ...
    private function scan_callback($m, $replace = false)
    {
        ...
        if (is_array($attr)) {
            if (is_array($attr['options'])) {
                if ($this->tag_type_supports($tag, 'name-attr')
                    ...
                    if (! cfredux_is_name($scanned_tag['name'])) {
            ...
            if (CFREDUX_USE_PIPE) {
                $pipes = new CFREDUX_Pipes($scanned_tag['raw_values']);
        ...
        $scanned_tag = apply_filters('cfredux_form_tag', $scanned_tag, $replace);

        $scanned_tag = new CFREDUX_FormTag($scanned_tag);
    ...
    private function parse_atts($text)
    {
        ...
        if (preg_match($pattern, $text, $match)) {
            ...
            if (! empty($match[2])) {
                ...
                $atts['values'] = cfredux_strip_quote_deep($matched_values[0]);
            }
        } else {
            $atts = $text;
        }
